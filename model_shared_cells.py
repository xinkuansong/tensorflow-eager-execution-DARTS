...
class DartSharedCells(tf.keras.Model):
    ...
    def __init__(self,
                 ...
                 n_filters,
                 reduction_channels):
        super(DartSharedCells, self).__init__()
        ...
        self.architecture_1 = get_architecture(first_two_cell=True, reduction_flag=False)
        self.architecture_2 = get_architecture(first_two_cell=False, reduction_flag=True)
        self.architecture_3 = get_architecture(first_two_cell=False, reduction_flag=False)

        self.cell_1 = Cell(True, False, self.n_filters, self.reduction_channels, self.architecture_1)
        self.cell_2 = Cell(True, False, self.n_filters, self.reduction_channels, self.architecture_1)

        #self.n_filters *= 2
        self.cell_3 = Cell(False, True, self.n_filters * 2, self.reduction_channels, self.architecture_2)
        self.cell_4 = Cell(False, False, self.n_filters * 2, self.reduction_channels, self.architecture_3)
        self.cell_5 = Cell(False, False, self.n_filters * 2, self.reduction_channels, self.architecture_3)

        #self.n_filters *= 2
        self.cell_6 = Cell(False, True, self.n_filters * 4, self.reduction_channels, self.architecture_2)
        self.cell_7 = Cell(False, False, self.n_filters * 4, self.reduction_channels, self.architecture_3)
        self.cell_8 = Cell(False, False, self.n_filters * 4, self.reduction_channels, self.architecture_3)
...
class Cell(tf.keras.Model):
    ...
    def __init__(self,
        ...
        self.edge_13 = Node(self.n_filters, self.reduction_stride, architecture[0])
        self.edge_23 = Node(self.n_filters, self.reduction_stride, architecture[1])

        self.edge_14 = Node(self.n_filters, self.reduction_stride, architecture[2])
        self.edge_24 = Node(self.n_filters, self.reduction_stride, architecture[3])
        self.edge_34 = Node(self.n_filters, self.stride, architecture[4])

        self.edge_15 = Node(self.n_filters, self.reduction_stride, architecture[5])
        self.edge_25 = Node(self.n_filters, self.reduction_stride, architecture[6])
        self.edge_35 = Node(self.n_filters, self.stride, architecture[7])
        self.edge_45 = Node(self.n_filters, self.stride, architecture[8])

        self.edge_16 = Node(self.n_filters, self.reduction_stride, architecture[9])
        self.edge_26 = Node(self.n_filters, self.reduction_stride, architecture[10])
        self.edge_36 = Node(self.n_filters, self.stride, architecture[11])
        self.edge_46 = Node(self.n_filters, self.stride, architecture[12])
        self.edge_56 = Node(self.n_filters, self.stride, architecture[13])

        self.output_edge_3 = Node(self.n_filters, self.stride, architecture[14])
        self.output_edge_4 = Node(self.n_filters, self.stride, architecture[15])
        self.output_edge_5 = Node(self.n_filters, self.stride, architecture[16])
        self.output_edge_6 = Node(self.n_filters, self.stride, architecture[17])
...
class Node(tf.keras.Model):
    ...
    def __init__(self,
        ...
        if self.reduction_stride == 1:
            ...
            self.conv1 = layers.Conv2D(filters=self.n_filters, kernel_size=(3, 3), strides=1, padding='same', activation='relu',
                                       kernel_initializer='he_normal')
            self.bn1 = layers.BatchNormalization()
            # self.dropout1 = layers.Dropout(self.DROPOUT)
            ...
            self.separable_conv3 = layers.SeparableConv2D(filters=self.n_filters, kernel_size=(3, 3), strides=1, padding='same',
                                        activation='relu', kernel_initializer='he_normal')
            self.bn3 = layers.BatchNormalization()
            # self.dropout3 = layers.Dropout(self.DROPOUT)

            self.separable_conv4 = layers.SeparableConv2D(filters=self.n_filters, kernel_size=(5, 5), strides=1, padding='same',
                                        activation='relu', kernel_initializer='he_normal')
            self.bn4 = layers.BatchNormalization()
            # self.dropout4 = layers.Dropout(self.DROPOUT)
    ...
    def call(self, inputs, training=True):
        ...
        if self.reduction_stride == 1:
            ...
            node_outputs = self.archlayer([y1, y2, y3, y4, y5, y6, y7, y8])
            ...
            node_outputs = self.archlayer([y1, y2, y3, y4])
...
def get_architecture(first_two_cell, reduction_flag):

    if (reduction_flag) and (not first_two_cell):
        reduction_stride = 2
    elif (not reduction_flag) and (first_two_cell):
        reduction_stride = 1000
    else:
        reduction_stride = 1
    stride = 1
    architecture = []
    architecture.append(get_arch(reduction_stride))
    architecture.append(get_arch(reduction_stride))

    architecture.append(get_arch(reduction_stride))
    architecture.append(get_arch(reduction_stride))
    architecture.append(get_arch(stride))
...
def get_arch(reduction_stride):

    if reduction_stride == 1:
        return ArchLayer(num_inputs=8)
    else:
        return ArchLayer(num_inputs=4)
